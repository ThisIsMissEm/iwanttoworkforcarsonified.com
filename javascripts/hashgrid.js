...
var hashgrid = function(set) {
  ...
  , leftGutter:     95
	...
	for (i = numGridLines - 1, m = 1, major=false; i >= 1; i--) {
    m++;
	  major = (m == options.major);
	  if (major) m = 0; 
		overlay.append('<div class="horiz'+(major ? " horiz-mjr" : "")+'"></div>');
	}
	...
	for(var i=options.count; i >= 0; --i){
	  var left = options.offset+i*options.width + ((i-1) * options.gutter*2);
	  if(i == 0){
	    left += options.gutter*2;
	  } else {
	    $('<div class="vert"/>').css({left: left+options.gutter*2+"px"}).prependTo(overlayVert);
	  }
	  $('<div class="vert"/>').css({left: left+"px"}).prependTo(overlayVert);
	}
	if(options.leftGutter > 0) {
	  $('<div class="vert vert-gutter"/>').css({left: options.leftGutter+"px"}).prependTo(overlayVert);
  }
  
  overlay.width(options.offset+options.count*options.width + ((options.count-1) * options.gutter*2)+options.gutter*2)
  
  $("<p>"+overlay.width()+"px &#x2192;</p>").css({position: "fixed", top: 0, left: overlay.width()-83, color: "#333", background: "#fff", padding: "0.2em 0 0.2em 0.2em", width: "80px", "text-align": "right"}).appendTo(overlay);
	...
	function keyupHandler(e) {
		...
		if(k == options.hideKey && overlayOn) {
	    overlay.fadeIn(200);
	  }
	}
